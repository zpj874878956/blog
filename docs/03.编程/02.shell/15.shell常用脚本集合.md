...
# 脚本运行日志（记录清理过程和异常）
SCRIPT_LOG="./clean_logs_script.log"
        ...
        for file in "$subdir"/*; do
            if [ -f "$file" ]; then
                ...
                while IFS= read -r line; do
                    if [[ -n "$line" ]]; then
                        raw_date=$(echo "$line" | awk '{print $2}')
                        echo "检查文件 $file 的行: '$line'，提取日期: '$raw_date'"  # ✅ 添加调试信息
                        if [[ -n "$raw_date" ]]; then
                            break
                        fi
                    fi
                ...
                if [[ $raw_date =~ ^[0-9]{4}/[0-9]{2}/[0-9]{2}$ ]]; then
                    formatted_date=$(echo "$raw_date" | tr -d '/')
                    dest_dir="${classified_dir}/${formatted_date}"
                    mkdir -p "$dest_dir"
                    cp "$file" "$dest_dir/"
                    echo "已归类文件: $file -> $dest_dir"
                else
                    echo "❌ 跳过无法识别日期格式的文件: $file (提取到的日期: '$raw_date')"
                fi
            fi
        done
    fi
...
echo "✅ 文件归类完成，归类路径：$classified_dir"
...
# 设置 NLU_MODEL 目录路径
NLU_MODEL="/path/to/NLU_MODEL"  # 请替换为实际路径
TRAIN_RECORD="$NLU_MODEL/train_record.json"

# 读取保留目录名（train_record.json 中列出的）
preserved_dirs=$(jq -r '.[]' "$TRAIN_RECORD" 2>/dev/null)
if [ $? -ne 0 ]; then
    echo "读取 train_record.json 失败，请检查文件格式或路径：$TRAIN_RECORD"
    exit 1
fi
...
preserved_array=()
while IFS= read -r line; do
    preserved_array+=("$line")
done <<< "$preserved_dirs"

# 进入 NLU_MODEL 目录
cd "$NLU_MODEL" || { echo "无法进入目录: $NLU_MODEL"; exit 1; }
...
for dir in */; do
    ...
    # 跳过 thesaurus 开头的目录
    [[ "$dir" == thesaurus* ]] && continue

    # 跳过保留目录
    skip=false
    for preserved in "${preserved_array[@]}"; do
    ...
    # 检查是否是7天前最后修改的目录
    if [ $(find "$dir" -prune -mtime +7) ]; then
        echo "删除目录: $dir"
        rm -rf "$dir"
    fi
done
