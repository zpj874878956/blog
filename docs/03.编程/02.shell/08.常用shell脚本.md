---
title: 常用shell脚本
date: 2023-02-24 15:30:36
permalink: /pages/8fdac1/
categories:
  - 编程
  - shell
tags:
...
# 设置日志文件路径
LOG_FILE="/var/log/rtp_file_process.log"
BASE_DIR="/rtp/docker-project/uploads/yuangong-ftp-bak"

# 获取当前日期用于日志标识
CURRENT_DATE=$(date '+%Y-%m-%d %H:%M:%S')

echo "=== 开始处理文件，时间: $CURRENT_DATE ===" >> "$LOG_FILE"
...
for date_dir in "$BASE_DIR"/*; do
    ...
    # 遍历未知子目录
    for sub_dir in "$date_dir"/*; do
        [ -d "$sub_dir" ] || continue

        # 查找 SQ 开头的 .RTP.gz.* 文件
        for file in "$sub_dir"/SQ*.RTP.gz.*; do
            [ -f "$file" ] || continue

            echo "处理目录: $sub_dir" >> "$LOG_FILE"
            echo "原始文件: $(basename "$file")" >> "$LOG_FILE"

            cd "$sub_dir" || continue

            # 获取标准文件名（去除最后一段日期）
            base_name=$(echo "$file" | sed -E 's/(\.RTP\.gz)\.[0-9]+$/\1/')
            mv "$(basename "$file")" "$(basename "$base_name")"

            # 解压缩
            gzip -d "$(basename "$base_name")"

            # 获取 .RTP 文件名
            rtp_file="${base_name%.gz}"

            if [ -f "$rtp_file" ]; then
                # 行数统计
                line_count=$(cat "$rtp_file" | wc -l)
                echo "文件 $rtp_file 行数: $line_count" >> "$LOG_FILE"
            else
                echo "解压后文件不存在: $rtp_file" >> "$LOG_FILE"
            fi
...
# VuePress docs 目录
DOCS_DIR="/root/blog/docs"

# 原地址
OLD_URL="https://chatpdf.quantchatai.top"

# 新地址
NEW_URL="https://pdf.chatquanttail.top"

# 查找并替换 markdown 文件中的图片地址
find "$DOCS_DIR" -type f -name "*.md" -print0 | while IFS= read -r -d '' file
do
...
原文链接：[常用shell脚本 | Gaoyufu &#39;s blog](https://www.gaoyufu.cn/pages/e617ab/)
