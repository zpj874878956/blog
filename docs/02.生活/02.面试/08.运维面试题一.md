...
有一次我们线上 Redis 内存持续上涨并触发告警，排查发现是部分 key 没有设置过期时间，导致数据一直堆积。

当时我先用 `INFO memory` 看整体内存使用情况，然后用 `redis-cli --bigkeys` 和 `SCAN` 去定位大 key 和无TTL的 key。

确认问题后，短期通过 SCAN 分批删除部分无效数据，避免使用 KEYS 这种阻塞命令；
 同时对大 key 做了拆分处理，减少单个 key 的内存占用。

后面做了两点优化：

- 所有缓存 key 强制加 TTL，避免数据无限增长
- 配置 maxmemory 和淘汰策略（allkeys-lru），防止再次打满

同时我们也加了内存和大 key 的监控，提前预警。

# 8、ELK+kafka日志阻塞问题，以及日志量过大导致的io问题解决方案

在 ELK + Kafka 架构中，如果出现日志阻塞或者IO打满，我一般会从Kafka、消费端和ES三个层面排查。

首先看Kafka是否有积压，如果有，会先扩容消费者或者增加分区提升消费能力，同时排查消费逻辑是否存在性能问题，比如是否是单条写入而不是批量写入。

其次看ES写入是否是瓶颈，比如磁盘IO是否打满，如果是的话，会通过Bulk批量写入、调整refresh_interval、临时降低副本数等方式优化写入性能。

同时也会从系统层面优化，比如使用SSD、调整IO参数，以及对日志采集端进行限流，避免流量过大。

如果数据量长期很大，还会考虑冷热分离或者归档策略，减少ES压力。
