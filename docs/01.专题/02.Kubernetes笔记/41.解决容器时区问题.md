...
如果你的集群有很多应用需要统一时区，推荐使用k8tz。它是一个准入控制器，可以自动为Pod注入时区配置，无需修改每个Deployment。

bash

```
# 1. 使用Helm安装k8tz，设置默认时区为北京时间
helm repo add k8tz https://k8tz.github.io/k8tz/
helm install k8tz k8tz/k8tz --set timezone=Asia/Shanghai

# 2. 之后，所有新建的Pod都会自动被注入北京时间配置
# 如果需要为已有Pod注入，可以执行：
kubectl get deploy -oyaml | k8tz inject --timezone=Asia/Shanghai | kubectl apply -f -
```
