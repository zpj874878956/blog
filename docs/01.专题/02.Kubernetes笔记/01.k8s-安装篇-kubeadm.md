...
```bash
nmcli con mod ens160 ipv4.addresses 192.168.51.40/24
nmcli con mod ens160 ipv4.gateway 192.168.51.1
nmcli con mod ens160 ipv4.dns "8.8.8.8"
nmcli con mod ens160 ipv4.method manual
nmcli con down ens160 && sudo nmcli con up ens160
```
...
```bash
192.168.171.136 k8s-master01
192.168.171.138 k8s-node01
192.168.171.139 k8s-node02 
```
...
. ./kubeinit.sh
-------------------------------------
kubeadm init --kubernetes-version=v1.32.0 --pod-network-cidr=10.244.0.0/16 --apiserver-advertise-address=192.168.80.76 --image-repository registry.aliyuncs.com/google_containers
------------------------------------------
  ...
  template:
    ...
    spec:
      ...
      - name: resume-container
        image: docker.cnb.cool/zzppjj/docker-images/resume:latest  # 建议使用具体版本标签
        ports:
        - containerPort: 3000
        resources:
          requests:
...
# 1. 拉取镜像（指定命名空间 k8s.io，这是 kubelet 使用的命名空间）
ctr -n k8s.io images pull m.daocloud.io/docker.io/istio/pilot:1.29.2
ctr -n k8s.io images pull m.daocloud.io/docker.io/istio/proxyv2:1.29.2

# 2. 打 tag 为 istioctl 默认拉取的标准路径
ctr -n k8s.io images tag \
  m.daocloud.io/docker.io/istio/pilot:1.29.2 \
  docker.io/istio/pilot:1.29.2

ctr -n k8s.io images tag \
  m.daocloud.io/docker.io/istio/proxyv2:1.29.2 \
